  ...
  watch: {
    value: {
      ...
      handler(value, oldValue) {
        ...
        const firstValToEmit = typeof value[0] === 'object' ? JSON.stringify(this.valueToUseForEmit(value[0])) : value[0];

        const firstVal = typeof value[0] === 'object' ? JSON.stringify(value[0]) : value[0];
        ...
        const selectedArray = [];
        value.forEach((item) => {
          const foundOption = this.options.find((option) => JSON.stringify(this.keyValue(option)) === JSON.stringify(this.keyValue(item)));
          if (foundOption) {
