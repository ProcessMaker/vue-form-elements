    ...
    watch: {
      validationData: {
        ...
        selectedOptions: {
          ...
          deep: true,
          handler(value) {
            console.log('cambio de selectedOptions: ', value);
          }
        }
      },
      options: {
    ...
    methods: {
      sendSelectedOptions() {
        ...
        if (this.options.renderAs !== 'dropdown') {
          if(this.options.allowMultiSelect) {
            valueToSend = this.optionsList.filter(opt => this.selectedOptions.findIndex(selOpt => selOpt === opt.value) >= 0)
          }
          else {
            valueToSend = this.optionsList.find(x => x[this.options.key] === valueToSend)
          }
        }

        console.log('Emitiendo: ',valueToSend);
